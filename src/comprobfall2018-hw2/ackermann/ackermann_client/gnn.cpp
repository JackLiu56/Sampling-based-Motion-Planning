...
{ 
    ...
    graph_nearest_neighbors_t::graph_nearest_neighbors_t(std::function<bool (proximity_node_t*,proximity_node_t*)> l,std::function<double (proximity_node_t*,proximity_node_t*)> d,
                                                         std::function<double (proximity_node_t*,proximity_node_t*)> dis2line                                                         )
    {
        distance_function = d;
        localvalid_function=l;
        dis2line_function=dis2line;
        added_node_id = 0;
    ...
    bool graph_nearest_neighbors_t::add_node( proximity_node_t* graph_node )
    {    
...
//        clear_added();
    ...
    proximity_node_t* graph_nearest_neighbors_t::basic_closest_search( proximity_node_t* state, double* the_distance, int* the_index )
    {
        ...
        for( int i=0; i<nr_samples; i++ )
        {
            ...
            index = i;
            double dis2line = dis2line_function(nodes[index],state);
            if(dis2line<min_distance)
            {
                min_distance = dis2line;
                min_index = index;
            }
        }
    ...
    bool graph_nearest_neighbors_t::isSameComponents(int node1_index, int node2_index)
    {
        ...
        for(int v = 0; v < nr_nodes; v++)
            visited[nodes[v]->get_index()] = false;
        DFSUtil(nodes[node1_index], visited);
        isSameComponent = visited[node2_index];
